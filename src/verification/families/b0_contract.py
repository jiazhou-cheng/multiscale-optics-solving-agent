...
_B0_CONTRACT = BenchmarkFamily(
        ...
        gate_disposition=GateDisposition(
            ...
            note=(
                ...
                "than assumed. Every declared ContractCode is separately shown "
                "reachable with a reason and a remedy -- stated without a count on "
                "purpose (CHE-142), because the count drifts whenever a code is added "
                "and a stale number reads as a coverage claim that was never "
                "re-checked. tests/test_contract_code_reachability.py is what holds "
                "it, and it fails on the code that arrives without a trigger.\n\n"
