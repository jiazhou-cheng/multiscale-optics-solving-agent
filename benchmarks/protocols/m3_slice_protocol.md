...
protocol in `benchmarks/protocols/slice_protocol.yaml`, which is the machine-readable
source of truth. Every number here comes from
`benchmarks/probes/slice_feasibility.py`,
`benchmarks/probes/carrier_phase_representation.py`, or a cited M1/M2 result.
...
```bash
./run.sh python benchmarks/probes/carrier_phase_representation.py
...
`benchmarks/reports/2026-08/sensor_handoff_convergence.md` §11, not from that record. Tracked in
CHE-63; disposition in `benchmarks/reports/2026-08/slice_cleanup_disposition.md` item 1.
...
```bash
./run.sh python benchmarks/probes/slice_feasibility.py
