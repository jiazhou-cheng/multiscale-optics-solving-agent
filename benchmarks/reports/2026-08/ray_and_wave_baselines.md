...
reference. Tolerances: `archive/benchmarks/gen1/benchmarks/L1-RAY-01/tolerances.yaml`.
...
`./run.sh python archive/benchmarks/gen1/benchmarks/verify_m1_independence.py` → **passed**, all five
checks:
...
`tests/test_m1_protocol.py` pins the enum — but `benchmarks/protocols/protocol.yaml` and
`benchmarks/protocols/m1_baseline_protocol.md` still describe only V1. **What V2 changed
