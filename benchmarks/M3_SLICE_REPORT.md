...
```bash
./run.sh python benchmarks/level2/L2-PSF-01/run_benchmark.py --output-dir outputs/M3/L2-PSF-01
./run.sh python benchmarks/level2/L2-PSF-01/evaluate.py outputs/M3/L2-PSF-01
...
`benchmarks/slice_protocol.yaml`'s frozen `airy_radius_um = 12.9746` is `1.22·λ/NA`
...
by this ticket lived in `M2_COUPLER_REPORT.md`'s prose, not in an executable
...
in `benchmarks/M3_9R_SENSOR_HANDOFF_REPORT.md` is evidence-complete from staged
