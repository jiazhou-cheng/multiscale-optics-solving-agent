...
- **Default active suite** (required after every change, 947 tests): `./run.sh pytest -q` — everything under `tests/`, including the 19 `slow` scientific tests (Monte Carlo/wave-to-ray convergence, coupler gradient and round-trip, and the wheel-install check). Last measured at the end of CHE-84 on a rebuilt image: **899 passed, 48 skipped in 193 s**. `-m "not slow"` is the fast subset CHE-53 called Tier A — **880 passed, 48 skipped, 43 s**. Subsystem subsets stay independently invocable and overlap by design: `-m optiland`, `-m chromatix`, `-m coupler`, `-m slow`. 48 of the 947 are `gpu`-marked and skip unless the session is GPU-dedicated. The count grew across the CHE-84 cleanup from 817: CHE-88 restored 46 archived PSF-oracle tests and added 12; CHE-89 added 14 flat-layout guards; CHE-90 added 9 dependency-direction guards; CHE-91 added 22 adapter characterization tests; CHE-93 added 7 schema-drift tests; and CHE-92/CHE-94 added 19 knowledge and architecture guards. All of the additions are static except the characterization suite.
- **On-demand tutorial suite** (60 tests, **~33 min**): `make test-tutorial`, i.e. `./run.sh pytest -q tests_tutorial`. Reproductions of upstream Optiland/Chromatix tutorials, in `tests_tutorial/cases/<solver>/` since CHE-92 moved them out of `knowledge/` — a gate on the *pinned dependency*, not on this repository's physics, and 76% of the old suite's runtime. Run it when a pin or `docker/` changes, before/after a substantial solver-integration change, or as a weekly sweep; not per PR. Active and maintained, just never collected unless the directory is named: `tests_tutorial/README.md`.
...
- **GPU** (48 tests): `./run.sh --gpu pytest -q -m gpu`, in its own session, on an attached CUDA device. Revalidated at the end of CHE-84 on a **rebuilt** `agent_solver_gpu` — **48 passed in 69 s** on one RTX A6000, matching CHE-72/CHE-73's 2026-08-20 figure of 70 s. If `--gpu` cannot start a container, read Trap 3: one faulted GPU on this host blocks all of them, and `run.sh` has a fallback (CHE-70 measured 48 passed in 77 s on the pre-SAX-removal image, which cleared CHE-60's outstanding dedicated pass). The suite now asserts real kernel execution *and* one coherent CUDA 12.6 dependency family, not device enumeration; `jax_platform_name` is neither set nor repaired anywhere. Setup, the resolved CUDA stack, and the traps: `docs/testing/gpu_environment.md`.
