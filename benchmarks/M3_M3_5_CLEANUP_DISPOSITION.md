...
`benchmarks/slice_protocol.yaml`, a coupler knowledge asset, or a Linear ticket
...
- `L2-PSF-01` loads `benchmarks/probes/m3r_sensor_handoff.py` **as a module** and
...
- `benchmarks/M3_9R_SENSOR_HANDOFF_REPORT.md` is evidence-complete from the
...
the four files that name it — `M3_SLICE_REPORT.md`, `M3_5_EXIT_REPORT.md`,
`PB7_COOKE_TRIPLET_PSF_REPORT.md`, `pb7_cooke_triplet_psf_comparison.py` — all
describe it as open.
...
- `benchmarks/manifest.yaml` and `benchmarks/M3_SLICE_REPORT.md` keep their "gate
...
`benchmarks/PB7_COOKE_TRIPLET_PSF_REPORT.md` ("Relation to known open issues")
and in `benchmarks/M3_5_EXIT_REPORT.md`. Adopting the PB7 result as sufficient
...
| Claim | Where | Verdict |
|---|---|---|
| `L2-PSF-01` does not meet the `1.0e-3` gate (`2.2e-3`–`2.5e-3`) | `manifest.yaml`, `M3_SLICE_REPORT.md`, `M3_5_EXIT_REPORT.md` | **Stands, unchanged.** Item 3 confirms it; only the ticket state was wrong |
| Sensor-side `C_RAY_TO_WAVE` handoff verified (CHE-38 verdict A) | `M3_9R_SENSOR_HANDOFF_REPORT.md` | Stands. Untouched by this ticket |
| CHE-38's consolidated record has never landed | `M3_SLICE_REPORT.md` L5, `M3_5_EXIT_REPORT.md` L1 | **Stands.** Now justified rather than merely noted — item 1 |
| `L2-PSF-01` provenance is from a dirty tree | `M3_SLICE_REPORT.md` L8, `M3_5_EXIT_REPORT.md` L2 | **Stands.** Precondition now satisfiable — item 2 |
| "CHE-48 is Done but the repository says the gate is unmet; one of the two is wrong" | `M3_5_EXIT_REPORT.md` L3 | **Resolved.** The ticket was wrong — item 3 |
| Tier A is "30.85 s, 478 passed / 21 skipped / 128 deselected" | `M3_5_EXIT_REPORT.md` PB1–PB3 table | **Stale as a present-tense claim.** True of PB3's tree; re-measured 42.78 s, 670 passed / 54 skipped / 173 deselected at `ee57e33`. The 21 record skips are unchanged; the other 33 are CHE-60/61's `gpu` quarantine, which did not exist when PB3 measured |
| "CHE-58 is still in Backlog" | `M3_5_EXIT_REPORT.md` R1 | **Stale.** CHE-58 and CHE-59 are both `Done` |
| "PB7's code is untracked" | `M3_5_EXIT_REPORT.md` R3 | **Discharged** at `ee57e33` |
| "M3.5 sits on an unmerged stack; `main` is at `ec55839`" | `M3_5_EXIT_REPORT.md` L10, R2 | **Discharged** by this ticket's merge |
| CHE-51 canceled with no reason recorded | `M3_5_EXIT_REPORT.md` R4 | **Resolved** — item 4 |
| No new physics claim in M3.5 | `M3_5_EXIT_REPORT.md` claim audit | Stands. CHE-62 adds none either |
...
- **L5–L9** of `M3_5_EXIT_REPORT.md` — PSF tolerances deferred (correctly, per
