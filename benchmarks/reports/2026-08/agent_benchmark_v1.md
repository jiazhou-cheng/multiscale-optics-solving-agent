...
Suite: `benchmarks/agents/` (task prompts, recorded expectations, the graded run).
Harness: `src/agent/benchmark_suite.py`. Full
...
Stated in full in `benchmarks/agents/README.md`; the decisions themselves:
...
1. **Context policy** — `cold` / `warm` / `guided`, **declared per task and
   ...
   *asserts* rather than sets, and errors if it disagrees with a task's
   declaration.
2. **Location** — `benchmarks/agents/`, opt-in by location, following CHE-67
...
| `./run.sh pytest -q benchmarks/agents` | — | **52 passed / 7.6 s** (opt-in) |
...
`./run.sh pytest -q`", which is satisfied exactly — `benchmarks/agents/` contributes
...
moving those 53 tests into `benchmarks/agents/` is a five-minute change — it just
