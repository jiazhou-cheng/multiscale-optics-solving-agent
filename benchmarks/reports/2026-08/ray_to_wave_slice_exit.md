...
> **`benchmarks/reports/2026-08/slice_cleanup_disposition.md`** — read that if this report and
...
CHE-58, full report: `benchmarks/reports/2026-08/cooke_triplet_psf_routes.md`; record:
`outputs/PB7/pb7_cooke_triplet_psf_comparison.json`; entry point
`./run.sh python benchmarks/probes/cooke_triplet_psf_routes.py`.
...
Meanwhile `manifest.yaml` and `reports/2026-08/ray_to_wave_slice.md` still record the gate as
...
`reports/2026-08/ray_to_wave_slice.md` were **correct and are unchanged** — nothing is promoted.
...
`benchmarks/probes/cooke_triplet_psf_routes.py` (1983 lines) is
...
to `benchmarks/reports/2026-08/cooke_triplet_psf_routes.md` so it is not lost with them.
...
`reports/2026-08/ray_to_wave_slice.md` gate language need updating), or it was closed as
...
`manifest.yaml` / `reports/2026-08/ray_to_wave_slice.md` language is **unchanged because it was
...
8. **The `1.0e-3` physical-correctness gate is inherited unmet, not silently
   ...
   PSF method. Read `benchmarks/reports/2026-08/slice_cleanup_disposition.md` before
