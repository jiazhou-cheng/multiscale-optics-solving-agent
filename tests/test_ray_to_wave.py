...
#: float64 has ~2.2e-16 relative resolution. A reconstruction accumulates a
#: handful of transcendental evaluations and a length-N complex sum per output
#: pixel, so the achievable floor is that resolution times a small growth
#: factor -- derived here rather than tuned until the test passed.
FLOAT64_EPS = np.finfo(np.float64).eps


def _roundoff_bound(ray_count: int, magnitude: float = 1.0) -> float:
    """Round-off bound for a coherent sum of ``ray_count`` unit wavelets."""
    return 32.0 * FLOAT64_EPS * magnitude * max(ray_count, 1)
...
def test_collimated_bundle_reconstructs_the_exact_plane_wave(theta_x: float) -> None:
    ...
    sum is N * <n,d> * exp(i k d.r) with no residual position dependence."""
    ...
    bound = _roundoff_bound(bundle.count)
    assert error <= bound, f"error {error:.3e} exceeds round-off bound {bound:.3e}"

    # The projection factor is cos(theta), and it is reported, not hidden.
...
def test_reconstruction_is_independent_of_where_the_rays_are_launched() -> None:
    ...
    assert error <= _roundoff_bound(dense.count, magnitude=dense.count)
...
def test_superposed_modes_add_coherently() -> None:
    ...
    assert error <= _roundoff_bound(combined.count, magnitude=combined.count)
...
def _reference_and_perturbed(perturbation: Perturbation, *, theta_x: float = 0.25):
    ...
    return control, perturbed, expected, bundle.count


@pytest.mark.parametrize(
    "perturbation",
    [
        Perturbation(phase_sign=-1),
        Perturbation(apply_projection_factor=False),
        Perturbation(apply_oblique_ramp=False),
        Perturbation(transpose_axes=True),
    ],
    ids=["phase_sign", "projection_factor", "oblique_ramp", "axis_transpose"],
)
def test_each_removed_term_is_detected(perturbation: Perturbation) -> None:
    control, perturbed, expected, count = _reference_and_perturbed(perturbation)

    # The unperturbed control passes...
    assert float(np.max(np.abs(control.u - expected))) <= _roundoff_bound(count)
    # ...and the perturbation is caught by the same comparison.
    residual = float(np.max(np.abs(perturbed.u - expected))) / count
...
def test_projection_factor_omission_is_invisible_at_normal_incidence() -> None:
    ...
    on_axis_control, on_axis_perturbed, _, _ = _reference_and_perturbed(
        Perturbation(apply_projection_factor=False), theta_x=0.0
    )
...
def test_a_steep_angle_is_a_grid_limitation_not_a_physics_limitation() -> None:
    ...
    expected = (
        bundle.count * math.cos(theta) * _plane_wave(direction, WAVELENGTH_M, GRID, (fine, fine))
    )
    assert float(np.max(np.abs(field.u - expected))) <= _roundoff_bound(bundle.count)
...
def test_the_reconstructed_field_declares_its_own_provenance() -> None:
    ...
    assert "eq 2" in field.provenance["equation"]
