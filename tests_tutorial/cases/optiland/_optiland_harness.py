"""Shared harness for the repo-owned Optiland tutorial reproductions (CHE-57 / PB6).

Each `tNN_<slug>.py` module in this directory is a *repo-owned* reimplementation
of one tutorial from the official Optiland Tutorials & Recipes index
(https://www.optiland.org/tutorials), rewritten against the pinned
`optiland==0.6.0` install rather than executed as an upstream notebook.
...
Run one reproduction on its own::
    ...
    ./run.sh python tests_tutorial/cases/optiland/t01_singlet_lens.py
...
# Every reproduction must be headless: tutorials call `.draw()`/`plt.show()`
# freely and the container has no display. Set before any pyplot import.
...
def standalone_main(meta: TutorialMeta, run) -> int:
    ...
    parser.add_argument(
        "--write-expected",
        action="store_true",
        help="record this run into tests_tutorial/cases/optiland/expected/",
    )
...
def tutorial_module_paths() -> list[Path]:
    """Every ``tNN_*.py`` reproduction in this directory, in slug order."""
    here = Path(__file__).resolve().parent
    return sorted(p for p in here.glob("t[0-9][0-9]_*.py"))
